...
function simulateGame (testTitle, winner) {
  ...
  expect('isGameOver should return false at start of game', math.checkGameOver(), false)
  expect('whoWon should return 0 at start of the game', math.whoWon(), 0)

  var questionCount = math.numberOfQuestions()
  expect('numberOfQuestions should be greater than 0', questionCount, 0, '>')

  for (var i = 0; i < questionCount; ++i) {
    expect('currentQuestion should be equal to ' + i, math.completedQuestion, i)

    // if these tests fails don't continue as the logic below depends on them

    var answerCount = math.numberOfAnswers()
    var passed = expect('numberOfAnswers should be greater than 1', answerCount, 1, '>')
    if (!passed) {
      return
    }

    var answer = math.correctAnswer()
    ...
    // then guess correct
    if (shouldGuessCorrect) expect('playTurn should return true if correctAnswer', math.playTurn(answer), true)
    else expect('playTurn should return false if incorrect answer', math.playTurn(incorrectAnswer), false)
  }

  math.playTurn(0)
  console.log(math.completedQuestion + ' ' + questionCount)
  expect('playTurn should not be allow after gameover, currentTurn should not advance', math.completedQuestion, questionCount)

  expect('whoWon should return ' + winner + ' at end of the game', math.whoWon(), winner)
}
